...
# pstrat <- 'pstrat_gpd_2012_age_4_cat_20170119.RDS'
...
### trump support by age for each race chart

temp <- list()
for (race_lab in unique(poststrat_cells$race)) {
  temp[[race_lab]] <- summarise(group_by(poststrat_cells, state, age, race),
                                mean_prob = (sum(prob_hat_dem*N_voters)/ sum(N_voters)))
  temp[[race_lab]] <- filter(temp[[race_lab]], race == race_lab)
  temp[[race_lab]] <- select(temp[[race_lab]], state, age, mean_prob)
  temp[[race_lab]] <- spread(temp[[race_lab]], age, mean_prob)
  temp[[race_lab]] <- temp[[race_lab]][,c("state", sel$age)]
}

temp$Overall <- summarise(group_by(poststrat_cells, state, age),
                          mean_prob = (sum(prob_hat_dem*N_voters)/ sum(N_voters)))
temp$Overall <- select(temp$Overall, state, age, mean_prob)
temp$Overall <- spread(temp$Overall, age, mean_prob)
temp$Overall <- temp$Overall[,c("state", sel$age)]

pdf(paste0(prefix_path, "/rep_support_age_race.pdf"),30,24)
...
text(0,0, paste0(rep_candidate, "'s Share of Vote by Age"), cex = 5)
# plots
par_old <- par()$mar
for(i in 1:length(sel$state_reorder)) {
  par(mar = par_old/3)
...
### trump support by education for each race chart
...
pdf(paste0(prefix_path, "/rep_support_educ_race.pdf"),30,24)
layout(rbind(rbind(rep(1,9), matrix(c(seq(1:50)+1,rep(52,4)), nrow = 6, byrow = TRUE)),
             c(rep(0,5),rep(52,4)),rep(53,9)),
       widths=rep(5,9),
       heights=c(3.3, rep(5,7), 2),
...
# title
plot(0,0, type="n",xlab="",ylab="",axes=F)
text(0,0, paste0(rep_candidate, "'s Share of Vote by Education"), cex = 5)
# plots
par_old <- par()$mar
for(i in 1:length(sel$state_reorder)) {
  par(mar = par_old/3)
  # whites
  plot(0:4, filter(temp$White, state == sel$state_reorder[i])[-1],
       type = "l", lwd = 5, xlab="", ylab="",
       xaxt='n', yaxt='n', xlim = c(0,4), ylim = c(0,1),
       col = "#FF851B")
  # blacks
  lines(0:4, filter(temp$Black, state == sel$state_reorder[i])[-1],
        col = "#111111", lwd = 5)
  # hispanics
  lines(0:4, filter(temp$Hispanic, state == sel$state_reorder[i])[-1],
        col = "#FF4136", lwd = 5)
  # others
  lines(0:4, filter(temp$Other, state == sel$state_reorder[i])[-1],
        col = "#2ECC40", lwd = 5)
  # overall
...
pdf(paste0(prefix_path, "/rep_support_educ_race_national.pdf"),8,6)
par(mar = c(9, 5.1, 4.1, 2.1))
# whites
plot(0:4, temp$White,
     type = "l", lwd = 3, xlab="", ylab="", main = paste0(rep_candidate, "'s Share of Vote by Education"),
     xaxt='n', yaxt='n', xlim = c(0,4), ylim = c(0,0.75), bty = "l",
     col = "#FF851B")
# blacks
lines(0:4, temp$Black,
      col = "#111111", lwd = 3)
# hispanics
lines(0:4, temp$Hispanic,
      col = "#FF4136", lwd = 3)
# others
lines(0:4, temp$Other,
      col = "#2ECC40", lwd = 3)
# overall
lines(0:4, temp$Overall,
      col = "#0074D9", lwd = 3)
axis(side = 2, at = seq(0,0.75, length.out = 4), labels = c("0%", "25%", "50%", "75%"), las = 1, cex.axis = 1.2, tick = FALSE)
axis(side = 1, at = 0:4, labels = sel$educ, las = 2, cex.axis = 1.2)
title(ylab = paste0(rep_candidate, "'s Share of Vote"), line = 4, cex.lab = 1.3)
title(sub = paste0("Post Strat: ", pstrat), line = 8, cex.sub = 0.6, adj = 0)
legend(3,0.75, legend = c("Whites", "Blacks", "Hispanics", "Others", "Overall"), col = c("#FF851B", "#111111", "#FF4136", "#2ECC40", "#0074D9"), lty = 1, lwd = 3, bty = "n", cex = 0.85)
dev.off()
...
for (gender_lab in unique(poststrat_cells$gender)) {
  temp[[gender_lab]] <- summarise(group_by(poststrat_cells, state, educ, gender),
                                  mean_prob = (sum(prob_hat_dem*N_voters)/ sum(N_voters)))
...
pdf(paste0(prefix_path, "/rep_support_educ_gender.pdf"),30,24)
layout(rbind(rbind(rep(1,9), matrix(c(seq(1:50)+1,rep(52,4)), nrow = 6, byrow = TRUE)),
             c(rep(0,5),rep(52,4)), rep(53,9)),
       widths=rep(5,9),
       heights=c(3.3, rep(5,7), 2),
...
pdf(paste0(prefix_path, "/rep_support_educ_gender_gap.pdf"),30,24)
layout(rbind(rbind(rep(1,9), matrix(c(seq(1:50)+1,rep(52,4)), nrow = 6, byrow = TRUE)),
             c(rep(0,5),rep(52,4)), rep(53,9)),
       widths=rep(5,9),
       heights=c(3.3, rep(5,7), 2),
...
# title
plot(0,0, type="n",xlab="",ylab="",axes=F)
text(0,0, "Gender Gap by Education (Men minus Women)", cex = 5)
# plots
par_old <- par()$mar
for(i in 1:length(sel$state_reorder)) {
  par(mar = par_old/3)
...
temp <- list()
for (gender_lab in unique(poststrat_cells$gender)) {
  temp[[gender_lab]] <- summarise(group_by(poststrat_cells, state, age, gender),
                                  mean_prob = (sum(prob_hat_dem*N_voters)/ sum(N_voters)))
  temp[[gender_lab]] <- filter(temp[[gender_lab]], gender == gender_lab)
...
pdf(paste0(prefix_path, "/rep_support_age_gender_gap.pdf"),30,24)
layout(rbind(rbind(rep(1,9), matrix(c(seq(1:50)+1,rep(52,4)), nrow = 6, byrow = TRUE)),
             c(rep(0,5),rep(52,4)), rep(53,9)),
       widths=rep(5,9),
       heights=c(3.3, rep(5,7), 2),
...
# title
plot(0,0, type="n",xlab="",ylab="",axes=F)
text(0,0, "Gender Gap by Age (Men minus Women)", cex = 5)
# plots
par_old <- par()$mar
for(i in 1:length(sel$state_reorder)) {
  par(mar = par_old/3)
...
### turnout by education for each race
...
for (race_lab in unique(poststrat_cells$race)) {
  ...
  temp[[race_lab]] <- filter(temp[[race_lab]], race == race_lab)
  temp[[race_lab]] <- select(temp[[race_lab]], state, educ, turnout)
  temp[[race_lab]] <- spread(temp[[race_lab]], educ, turnout)
  temp[[race_lab]] <- temp[[race_lab]][,c("state", sel$educ)]
}

temp$Overall <- summarise(group_by(poststrat_cells, state, educ),
                          turnout = (sum(N_voters)/ sum(N_elig)))
temp$Overall <- select(temp$Overall, state, educ, turnout)
temp$Overall <- spread(temp$Overall, educ, turnout)
temp$Overall <- temp$Overall[,c("state", sel$educ)]

pdf(paste0(prefix_path, "/turnout_rep_support_educ_race.pdf"),30,24)
layout(rbind(rbind(rep(1,9), matrix(c(seq(1:50)+1,rep(52,4)), nrow = 6, byrow = TRUE)),
             c(rep(0,5),rep(52,4)),rep(53,9)),
       widths=rep(5,9),
       heights=c(3.3, rep(5,7), 2),
...
# title
plot(0,0, type="n",xlab="",ylab="",axes=F)
text(0,0, "Voter Turnout by Education", cex = 5)
# plots
par_old <- par()$mar
for(i in 1:length(sel$state_reorder)) {
  par(mar = par_old/3)
  ...
  plot(0:4, filter(temp$White, state == sel$state_reorder[i])[-1],
       ...
       xaxt='n', yaxt='n', xlim = c(0,4), ylim = c(0.1,0.9),
       col = "#FF851B")
  # blacks
  lines(0:4, filter(temp$Black, state == sel$state_reorder[i])[-1],
        col = "#111111", lwd = 3)
  ...
  lines(0:4, filter(temp$Overall, state == sel$state_reorder[i])[-1],
        col = "#0074D9", lwd = 3)
  title_col <- "black"
  text(2, 0.8, sel$state_reorder_lab[i], cex = 3, col = title_col)
  if(i %in% (c(47, 48, 49, 50, 51, 43, 44, 45, 46)-1)) {
    axis(side = 1, at = 0:4, labels = sel$educ, las = 2, cex.axis = 2.5)
  }
  if(i %in% (c(2, 11, 20, 29, 38, 47)-1)) {
    axis(side = 2, at = seq(0.1,0.9, length.out = 3), labels = c("10%", "50%", "90%"), las = 1, cex.axis = 2.5, tick = FALSE)
  }
}
par(bg=NA) 
plot(0,0, type="n",xlab="",ylab="",axes=F)
...
for (race_lab in unique(poststrat_cells$race)) {
  temp[[race_lab]] <- summarise(group_by(poststrat_cells, state, age, race),
                                turnout = (sum(N_voters)/ sum(N_elig)))
  temp[[race_lab]] <- filter(temp[[race_lab]], race == race_lab)
  temp[[race_lab]] <- select(temp[[race_lab]], state, age, turnout)
  temp[[race_lab]] <- spread(temp[[race_lab]], age, turnout)
  temp[[race_lab]] <- temp[[race_lab]][,c("state", sel$age)]
}

temp$Overall <- summarise(group_by(poststrat_cells, state, age),
                          turnout = (sum(N_voters)/ sum(N_elig)))
temp$Overall <- select(temp$Overall, state, age, turnout)
temp$Overall <- spread(temp$Overall, age, turnout)
temp$Overall <- temp$Overall[,c("state", sel$age)]


pdf(paste0(prefix_path, "/turnout_rep_support_age_race.pdf"),30,24)
layout(rbind(rbind(rep(1,9), matrix(c(seq(1:50)+1,rep(52,4)), nrow = 6, byrow = TRUE)),
             c(rep(0,5),rep(52,4)),rep(53,9)),
       widths=rep(5,9),
       heights=c(3.3, rep(5,7), 2),
...
# title
plot(0,0, type="n",xlab="",ylab="",axes=F)
text(0,0, "Voter Turnout by Age", cex = 5)
# plots
par_old <- par()$mar
for(i in 1:length(sel$state_reorder)) {
  par(mar = par_old/3)
  ...
  plot(0:3, filter(temp$White, state == sel$state_reorder[i])[-1],
       type = "l", lwd = 3, xlab="", ylab="",
       xaxt='n', yaxt='n', xlim = c(0,3), ylim = c(0.15,0.85),
       col = "#FF851B")
  # blacks
  lines(0:3, filter(temp$Black, state == sel$state_reorder[i])[-1],
        col = "#111111", lwd = 3)
  ...
  lines(0:3, filter(temp$Overall, state == sel$state_reorder[i])[-1],
        col = "#0074D9", lwd = 3)
  title_col <- "black"
  text(1.5, 0.8, sel$state_reorder_lab[i], cex = 3, col = title_col)
  if(i %in% (c(47, 48, 49, 50, 51, 43, 44, 45, 46)-1)) {
    axis(side = 1, at = 0:3, labels = sel$age_label, las = 2, cex.axis = 2.5)
  }
  if(i %in% (c(2, 11, 20, 29, 38, 47)-1)) {
    axis(side = 2, at = seq(0.15,0.85, length.out = 3), labels = c("15%", "50%", "85%"), las = 1, cex.axis = 2.5, tick = FALSE)
  }
}
par(bg=NA) 
plot(0,0, type="n",xlab="",ylab="",axes=F)
...
for (gender_lab in unique(poststrat_cells$gender)) {
  temp[[gender_lab]] <- summarise(group_by(poststrat_cells, state, educ, gender),
                                  turnout = (sum(N_voters)/ sum(N_elig)))
  temp[[gender_lab]] <- filter(temp[[gender_lab]], gender == gender_lab)
  temp[[gender_lab]] <- select(temp[[gender_lab]], state, educ, turnout)
  temp[[gender_lab]] <- spread(temp[[gender_lab]], educ, turnout)
  temp[[gender_lab]] <- temp[[gender_lab]][,c("state", sel$educ)]
}
...
pdf(paste0(prefix_path, "/turnout_rep_support_educ_gender.pdf"),30,24)
layout(rbind(rbind(rep(1,9), matrix(c(seq(1:50)+1,rep(52,4)), nrow = 6, byrow = TRUE)),
             c(rep(0,5),rep(52,4)), rep(53,9)),
       widths=rep(5,9),
       heights=c(3.3, rep(5,7), 2),
