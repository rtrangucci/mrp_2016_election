...
for(i in c("red", "blue", "swing")) {
  for (gender_lab in unique(poststrat_cells$gender)) {
    temp[[gender_lab]] <- summarise(group_by(filter(poststrat_cells, state %in% sel[[i]]), age, gender),
                                    mean_prob = (sum(prob_hat_dem*N_voters)/ sum(N_voters)))
...
for(i in c("red", "blue", "swing")) {
  for (gender_lab in unique(poststrat_cells$gender)) {
    temp[[gender_lab]] <- summarise(group_by(filter(poststrat_cells, state %in% sel[[i]]), educ, gender),
                                    mean_prob = (sum(prob_hat_dem*N_voters)/ sum(N_voters)))
    temp[[gender_lab]] <- filter(temp[[gender_lab]], gender == gender_lab)
...
pdf(paste0(prefix_path, "rep_support_rbs_pooled.pdf"),12,8, pointsize = 18)
...
for(i in c("red", "blue", "swing")) {
  for (gender_lab in unique(poststrat_cells$gender)) {
    temp[[gender_lab]] <- summarise(group_by(filter(poststrat_cells, state %in% sel[[i]]), age, educ, gender),
                                    mean_prob = (sum(prob_hat_dem*N_voters)/ sum(N_voters)))
    temp[[gender_lab]] <- filter(temp[[gender_lab]], gender == gender_lab)
    temp[[gender_lab]] <- spread(temp[[gender_lab]], educ, mean_prob)
...
pdf(paste0(prefix_path, "rep_support_rbs_pooled_4plot.pdf"),18,6, pointsize = 18)
...
pdf(paste0(prefix_path, "rep_support_rbs_pooled_4plot_ts.pdf"),18,6, pointsize = 18)
layout(matrix(c(1,2,3,4,5,5,0,0,6,6,0,0), ncol = 4, byrow = TRUE),
       widths=c(5,5,5,5),
       heights=c(5,1,1),
       respect = T)
...
for(i in 1:length(sel$age)) {
  plot(0, 0, type = "n",
       ylim = c(0.1,0.8), xlim = c(0,4), yaxt = "n", xaxt = "n", bty = "l",
       xlab = "", ylab = if(i==1) paste0(rep_candidate, " Share of Two-Party Vote") else "", cex.lab = 1.2, yaxs = "i", cex.main = 1.5,
       main = sel$age_label[i])
  axis(1, at = c(0,1,3), labels = c("< HS", "HS", "College"), cex.axis = 1.2)
  axis(1, at = c(2,4), labels = c("Some\nCollege", "Post-\ngrad"), cex.axis = 1.2,
       mgp=c(3, 2, 0))
  axis(2, at = seq(0.1, 0.8, by = 0.1), labels = paste0(seq(0.1, 0.8, by = 0.1)*100,"%"),
       cex.axis = 1, las = 2)
